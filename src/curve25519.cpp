...
std::array<unsigned char, 32> to_curve25519_pubkey(uspan ed25519_pubkey) {
...
std::array<unsigned char, 32> to_curve25519_seckey(uspan ed25519_seckey) {
...
LIBSESSION_C_API bool session_to_curve25519_pubkey(
    ...
    try {
        auto curve_pk = session::curve25519::to_curve25519_pubkey(uspan{ed25519_pubkey, 32});
        std::memcpy(curve25519_pk_out, curve_pk.data(), curve_pk.size());
        return true;
    } catch (...) {
        return false;
...
LIBSESSION_C_API bool session_to_curve25519_seckey(
    ...
    try {
        auto curve_sk = session::curve25519::to_curve25519_seckey(uspan{ed25519_seckey, 64});
        std::memcpy(curve25519_sk_out, curve_sk.data(), curve_sk.size());
        return true;
    } catch (...) {
        return false;
